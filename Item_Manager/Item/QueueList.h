...
// 크기를 재설정한다.
template<typename T>
void QueueList<T>::resize(int max) {
	delete[] m_pItems;
	m_nMaxQueue = max;
	m_pItems = new T[max]; //m_pItems에 크기가 max인 배열을 만든다.
	m_iFront = m_nMaxQueue - 1;
...
template<typename T>
void QueueList<T>::ResetCurrentPointer() {
...
template<typename T>
int QueueList<T>::Replace(T item) {

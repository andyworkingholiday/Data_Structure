...
public:
	...
	*	@return	삭제 실패 시 0을 반환하고, 성공하면 1을 반환.
	*/
	int Delete(T& item);
	...
	*	@pre	item을 입력받는다.
	*	@post	새로운 정보로 교체된다.
	*	@return	교체 실패 시 0을 반환하고, 성공하면 1을 반환.
	*/
	int Replace(T& item);
	...
	*/
	...
	void operator=(DoublySortedLinkedList& data) {
		MakeEmpty();
		
		if (data.GetLength()) {
			DoublyIterator<T> itor(data);
			T item;
...
void DoublySortedLinkedList<T>::Print() {
	...
	while (itor.m_pCurPointer != m_pLast) {
		tmpWalker = itor.m_pCurPointer->data;
		cout << tmpWalker << endl;
		itor.Next();
	}
}

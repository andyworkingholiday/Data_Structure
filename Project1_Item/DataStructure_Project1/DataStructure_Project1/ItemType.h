...
public:
	...
	*/
	ItemType(int inUsage, int inSID, int inCID, string inRtime, int inPday, string inBrand, string inName, int inNum, bool inHeart, int inItemNum) {
	...
	*	@param	inItemNum	 Primary KeyNumber
	*/
	void SetRecord(int inUsage, int inSID, int inCID, string inRtime, int inPday, string inBrand, string inName, int inNum, bool inHeart, int inItemNum) {
		SetUsage(inUsage);
		SetStorageId(inSID);
	...
	*/
	...
	friend ostream& operator<<(ostream& os, const ItemType &data) {
		...
		os << "\tContainer	: " << data.GetContainerId() << "번 째 Container" << endl;
...
private:
	int m_Usage; //물품의 종류
	int m_StorageId; //물품이 저장될 위치
	int m_ContainerId; //위치에 저장되는 컨테이너
	string m_RegisterTime; // 물품을 프로그램에 등록한 시간 YYYYMMDDHHMMSS
	int m_Pday; // 물품 구입년월일
	string m_Brand; // 물품의 회사명
	string m_Name; //물품명
	int m_Num; // 구매한 물품의 수
	bool m_Heart; // 자주찾거나 소중한 물건일 경우 하트표시

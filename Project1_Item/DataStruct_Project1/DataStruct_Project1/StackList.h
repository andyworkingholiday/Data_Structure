...
public:
	...
	void operator=(const StackList& data) {
		delete[] m_pItems;
		int Length = (data.m_iTop) + 1;
		m_pItems = new T[maxStack];
		for (int i = 0; i < Length; i++) {
			Push(data.m_pItems[i]);
		}
	}
...
//크기를 재설정 합니다.
template<typename T>
void StackList<T>::resize(int max) {
	delete[] m_pItems;
	m_nMaxStack = max;
	m_pItems = new T[m_nMaxStack];
	m_iTop = -1;
...
template<typename T>
void StackList<T>::ResetCurrentPointer() {
	m_CurPointer = -1; // 포인터를 -1로 초기화 해준다.
}

template<typename T>
int StackList<T>::GetNextItem(T& item) {
	m_CurPointer++;
...
template<typename T>
int StackList<T>::Replace(T item) {

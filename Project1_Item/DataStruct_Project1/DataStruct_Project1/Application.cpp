...
int Application::GetCommand() {
	...
	cout << "\t   15 : Display Recent List " << endl;
	cout << "\t   0 : Quit" << endl;
...
int Application::AddItemToMasterListFromKB() {
	if (MasterList.IsFull()) {
		...
		int index = PlaceList.Get(cdata); //cdata의 Container Id 와 일치하는 Container list의 type을 찾는다.
		...
		if (index) { 
			...
			PlaceList.Replace(cdata); //갱신된 cdata로 Replace 한다.
		}

		else { 
			cdata.AddItem(sdata);
			PlaceList.Add(cdata); //찾지 못했을 때 새로운 Container Id를 가진 type을 추가한다.
		}
...
int Application::DeleteItemFromMaster() {
	...
	if (MasterList.Delete(data)) { // MasterList에 해당 data 있을 시 삭제하면서 동시에 data 읽어옴
		...
		PlaceList.Get(Cdata);  // Cdata의 장소를 Set 하고 장소에 따른 아이템을 찾는다.
		Cdata.DeleteItem(Sdata); // 반드시 data가 있을 수 밖에 없으므로 해당 Sdata 삭제
		PlaceList.Replace(Cdata); //삭제완료 후 Cdata로 Container List 새로 갱신
...
int Application::ReplaceItemFromMaster() {
	...
	if (MasterList.RetrieveByBS(data) != -1 ) { // MasterList에 해당 data 있을 시 
		...
		PlaceList.Replace(Cdata); // 바뀐 Cdata로 Update
...
int Application::AddPhoto() {
	...
	if (PlaceList.Get(Cdata)) {
		...
		PlaceList.Replace(Cdata); //Cdata로 해당 Container List 대체
...
int Application::DeletePhoto() {
	...
	Cdata.SetContainerIdFromKB();
	if (PlaceList.Get(Cdata)) { // 컨테이너 찾을 시
		Cdata.DisplayAllPhoto(); //해당 Container List의 모든 사진 목록 출력
		cout << endl;
		string s;
		cout << "\n\tPhoto Name : ";
		...
		if (Cdata.DeletePhoto(s)) { //일치하는 string의 Container List의 사진 삭제
			PlaceList.Replace(Cdata); // 삭제 한 후 Contanier List로 갱신

...
// 내가 바꾸려고 하는 노드를 찾는 recursive 함수
template<typename T>
void Replace(Node<T>*& root, T& item)
{
	if (item < root->data)				// root노드값보다 item노드가 작을 때
		Replace(root->left, item);		// 왼쪽노드로 가서 replace함수 호출
	else if (item > root->data)			// root노드값보다 item노드가 클 때
		Replace(root->right, item);		// 오른쪽노드로 가서 replace함수 호출
	else {
		root->data = item;
	}	// 찾고자 하는 값이 일치하는 경우 deletenode 함수 호출
}

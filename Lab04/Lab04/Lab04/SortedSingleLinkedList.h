...
#ifndef _SORTEDSINGLELINKEDLIST_H
#define _SORTEDSINGLELINKEDLIST_H
...
public:
	...
	*/
	...
	void operator=(SortedSingleLinkedList& data) {
		// 기존 노드 초기화
		MakeEmpty();
	...
	friend ostream& operator<<(ostream& os, SortedSingleLinkedList& data) {
		T item;
		data.ResetList();
		
		int len = data.GetLength();
		while (len--) { //담긴 갯수만큼 반복
			data.GetNextItem(item);
			os << item << endl; // 아이템 출력
		}
		return os;
...
template <typename T>
void SortedSingleLinkedList<T>::MakeEmpty() {
...
template <typename T>
int SortedSingleLinkedList<T>::GetLength() const {
	return m_nLength;
}
...
template <typename T>
int SortedSingleLinkedList<T>::Add(T& item) {
	// 리스트 초기화
	ResetList();
...
template <typename T>
int SortedSingleLinkedList<T>::Delete(T& item) {
...
template <typename T>
int SortedSingleLinkedList<T>::Replace(T& item){
	bool moreToSearch, found;
...
template <typename T>
int SortedSingleLinkedList<T>::Get(T& item){
	bool moreToSearch, found;
	NodeType<T>* location;	//변수 선언
...
template <typename T>
void SortedSingleLinkedList<T>::ResetList(){
	// current pointer 초기화
	m_pCurPointer = nullptr;
}
...
template <typename T>
void SortedSingleLinkedList<T>::GetNextItem(T& item) {
...
template <typename T>
void SortedSingleLinkedList<T>::Print() {
	T tmpWalker;
	ResetList();
	int len = m_nLength;
	while (len--) {
